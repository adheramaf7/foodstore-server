...
modelSchema.plugin(AutoIncrement, { inc_field: 'order_number' });

modelSchema.virtual('items_count').get(function () {
  return this.order_items.reduce((total, item) => {
    return total + parseInt(item.qty);
  }, 0);
});

modelSchema.post('save', async function () {
...
module.exports = model('Order', modelSchema);
