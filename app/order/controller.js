...
async function index(req, res, next) {
  ...
  try {
    ...
    let orders = await Order.find({ user: req.user._id }).limit(parseInt(limit)).skip(parseInt(skip)).populate('order_items').sort('-createdAt');
...
async function store(req, res, next) {
  ...
  try {
    ...
    orderItems.forEach((orderItem) => order.order_items.push(orderItem));
